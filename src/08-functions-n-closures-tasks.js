 ...
 *   getPolynom()      => null
 */
function getPolynom(...spread) {
  const arr = spread;
  return function polyFunc(unknown) {
    return arr.reduce((b, a, index) => (unknown ** (arr.length - index - 1)) * a + b, 0);
  };
}
