...
function getSumBetweenNumbers(n1, n2) {
  const arr = Array(n2 - n1 + 1).fill().map((_, idx) => n1 + idx);
...
function doRectanglesOverlap(rect1, rect2) {
  const topLeft1 = [rect1.top, rect1.left];
  const bottomRight1 = [rect1.top + rect1.height, rect1.left + rect1.width];
  const topLeft2 = [rect2.top, rect2.left];
  const bottomRight2 = [rect2.top + rect2.height, rect2.left + rect2.width];
...
function isInsideCircle(circle, point) {
  const sumX = (point.x - circle.center.x) ** 2;
  const sumY = (point.y - circle.center.y) ** 2;
  const distance = (sumX + sumY) || 0;
...
function getDigitalRoot(num) {
  const splitReduce = (x) => x.toString().split('').reduce((a, b) => Number(a) + Number(b), 0);
  let test = splitReduce(num);
  while (test > 9) {
    test = splitReduce(test);
  }
  return test;
}
...
function getMatrixProduct(m1, m2) {
  const a = m1;
  const b = m2;
  return a.map((r) => b[0].map((_, j) => b.map((s) => s[j]).reduce((s, n, k) => s + n * r[k], 0)));
}
